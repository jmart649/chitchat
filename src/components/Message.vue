<template>
    <div @click.stop="copyMessage" class="container mx-auto flex my-4 rounded-full transform transition cursor-pointer hover:scale-105" :class="isUser ? 'bg-red-700' : 'bg-blue-700'">
        <div>
            <img
        ...
        <div class="flex flex-grow" >
            <div class="flex flex-grow items-center justify-between mx-4 font-thin text-xl" >
                <p> {{ message.text }} </p>
                <p class="text-sm font-thin italic" > {{ message.userName }}</p>
            </div>
        </div>
    </div>
